pipeline {
    ...
    environment {
        ...
        K8S_NAMESPACE = 'developement'

        // Git Branch variable
        GIT_BRANCH = 'dev' // <--- Set teh git branch here (e.g., 'main', 'dev')
    }
    ...
    stages {
        stage('Checkout Source Code') {
            steps {
                ...
                dir("${env.FRONTEND_GITHUB_REPO}") {
                    // Using the GIT_BRANCH variable
                    git branch: env.GIT_BRANCH, url: "https://github.com/brink-ops/${env.FRONTEND_GITHUB_REPO}.git" //, credentialsId: 'your-github-pat-id'
                ...
                dir("${env.BACKEND_GITHUB_REPO}") {
                    // Using the GIT_BRANCH variable
                    git branch: env.GIT_BRANCH, url: "https://github.com/brink-ops/${env.BACKEND_GITHUB_REPO}.git" //, credentialsId: 'your-github-pat-id'
        ...
        stage('Build and Push Docker Images') {
            steps {
                script {
                    // Define a variable for the combined tag for clarity
                    def full_tag = "${env.BUILD_NUMBER}-${env.GIT_BRANCH}" // Using '-' as a separator, you can use '_' if preferred
                    ...
                    // Build and push Frontend Docker image from its repo directory
                    dir("${env.FRONTEND_GITHUB_REPO}") {
        ...
        stage('Deploy to EKS') {
            steps {
                script {
                    // IMPORTANT: You'll also need to update your deployment YAMLs to use the new tag format
                    def image_tag_for_eks = "${env.BUILD_NUMBER}-${env.GIT_BRANCH}" // Ensure this matches the tag pushed to Docker Hub
                        ...
                        spec:
                          ...
                          template:
                            ...
                            spec:
                              containers:
                              - name: py-test-front
                                image: ${DOCKER_HUB_FRONT_IMAGE}:${image_tag_for_eks} # <--- UPDATED IMAGE TAG
                                ports:
                                - containerPort: 80
                    """
                        ...
                        spec:
                          ...
                          template:
                            ...
                            spec:
                              containers:
                              - name: py-test-back
                                image: ${DOCKER_HUB_BACK_IMAGE}:${image_tag_for_eks} # <--- UPDATED IMAGE TAG
                                ports:
                                - containerPort: 5000
                    """
